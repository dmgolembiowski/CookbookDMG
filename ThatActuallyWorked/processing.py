...
def process_descriptions(children, imitmidx):
    """
    ...
    for row in imitmidx.itertuples():
        item_no = row[1].strip(' ')
        if item_no in list_o_item_no:
            description_one = row[2]
            description_two = row[3]
...
def get_children(current_item, bmprdstr_sql):
    children = { 0: { 
        'ID':0,
    ...
    def next_generation(ID = 0, parent_ID = -1, level=0, df=bmprdstr_sql):    
        """
        ...
        these objects are not hashable or subscriptable.
     
        Therefore `generations()` returns an implicitly-ordered dictionary that where each material's
        ID returns a dictionary of its data template. Dictionaries make it simple to lookup children
        ...
        level = children[current_parent_ID]['level'] + 1
        ID += 1
        ...
        """ 
        After the `while frontier:` statement, there are no more child nodes
        to explore that are one level deeper than the current parent. 
        So, we must add the current parent's ID to the set of completely
        explored predecessors. 
        """
        while frontier:
            print('frontier: ', frontier)
            this_child = frontier.popleft()
            print('this_child',this_child)
            ...
            input()
            """
            #if current_child['ID'] not in children[current_parent_ID]['kids_IDs']:
            children[current_parent_ID]['kids_IDs'].append(current_child['ID'])
            print('children[current_parent_ID]["kids_IDs"].append(current_child["ID"]:',children[current_parent_ID]['kids_IDs'])
        ...
        while visited:
            grand_child = visited.pop()
            next_generation(ID=grand_child['ID'], parent_ID=grand_child['ID'], level=grand_child['level'])
        
    next_generation()
    return children

def main(imitmidx_sql, iminvloc_sql, bmprdstr_sql):
    """
    ...
    children = get_children(parent_item, bmprdstr_sql)
    ...
    billOfMaterials = billOfMaterials[['ID', 'item_no', 'description', 'qty_per_par', 'avg_cost', 'last_cost', 'level', 'parent_ID', 'kids_IDs']]
    return billOfMaterials

'''
if __name__ == "__main__":
    print("Warning: This file should not be directly run.")
    print("It is designed to be called from `load.main()` which is able to pass")
    print("pandas.DataFrame objects: imitmidx_sql, iminvloc_sql, bmprdstr_sql")
    # For satisfying Pylint
    main()'''